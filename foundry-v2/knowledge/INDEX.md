...
| **Problem discovery** | `problem-discovery/SKILL.md` | The project's problem is *assumed rather than verified* — the brief names a *solution* to build ("build X") rather than a verified pain, or the problem is asserted without current evidence. Determines which problem in a domain is worth solving *before* building. Recognize via the file's §1 check; if the problem is already given and verified as fixed, never load. Runs **first**, upstream of the regulated-domain check and the build. | Prose standard (discipline — no code) | Plan (upstream) | No |
