...
>
> **Where it sits.** `~/Foundry/foundation/RED_LINES.md`. The safety-critical lines are enforced *technically* by the knowledge branch — `knowledge/guardrails/SKILL.md` (untrusted input, leakage, tool misuse), `knowledge/deployment/SKILL.md` (secrets), and `knowledge/regulatory-architecture/SKILL.md` (external regulatory rules) — when a task triggers them.
>
