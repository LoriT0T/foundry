...
STEP 4 — Select the capabilities you need.
  ...
  apply it. (Not optional: an ASSUMED answer to birth question 1 makes
  knowledge/problem-discovery/SKILL.md mandatory and FIRST — run it before the
  rest; a YES to birth question 7 makes
  knowledge/regulatory-architecture/SKILL.md mandatory in that set.)
...
1. **Problem verified or assumed? (mandatory recognition — runs first)** — is the problem this agent will solve *given and verified*, or *assumed*? You cannot tell from inside the foundation, and a brief usually arrives naming a *solution* ("build X"), not a verified problem. Apply the §1 recognition check in `knowledge/problem-discovery/SKILL.md`. If the problem is **assumed** — named as a solution, asserted without evidence, or merely the first idea — that capability is **mandatory**: load it and run its discovery playbook (§3) to find the domain's real highest-value problem *before* answering the rest of these questions and *before* the regulated-domain check (Q7). Its output — a verified **problem statement** — becomes the Goal in Q2 (full routing in `knowledge/INSTANTIATION_GUIDE.md` STEP 1). If the problem is genuinely **given and verified** as fixed, proceed unchanged; it never loads. This runs *first* because you must know *which* problem you are solving before you can ask whether *that* problem is regulated (Q7) or build anything for it. The failure this catches is **building the wrong thing well.**
2. **Goal** — in one sentence (for an assumed problem, this is the verified problem statement Q1 produced). And what does *"this is working well"* look like, concretely and verifiably?
3. **Judgment vs. determinism** — where does the task need real judgment (reserve the model there) and where is it just rules and math (make that deterministic and reliable)?
4. **Data** — what data sources will it work with? Name each *distinct knowledge base*. What language / domain does that data sit in?
5. **Irreversibility** — what actions are risky or irreversible (money, public messages, deletions)? Those get a human gate or a hard coded limit — never a free hand.
6. **Cadence** — what rhythm does it run on, and how should it keep its principal informed?
7. **Regulated domain (mandatory recognition)** — does this project operate in a regulated domain? You cannot tell from inside the foundation: apply the §1 recognition check in `knowledge/regulatory-architecture/SKILL.md` against what the work touches and who can penalize it. If plausibly **YES**, that capability is **mandatory** — load it and run its research playbook *before* building (full routing in `knowledge/INSTANTIATION_GUIDE.md` STEP 1). A positive answer commits the agent not only to *checking* the discovered rules but to letting them *drive* the build (the capability's §5), discharged at the architecture step — not in this question. If **NO**, proceed unchanged; it never loads. The failure this catches is not a missed capability but an *unrecognized* regulated domain.
